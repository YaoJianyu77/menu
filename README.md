...
RecipeCard in `catalog.py` and the matching DOM renderer in `site/catalog.js` put the linked title first, an optional permitted image second, and time/cooking method last. Cuisine and meal-type labels are omitted from cards and detail headers; cuisine metadata still supports filters and search. Food Lion percentages are not displayed; compatibility filters use qualitative labels while keeping their existing thresholds. Missing images produce no placeholder. Image licensing and ingredient/cuisine classification are unchanged. The current corpus has no permitted image assets; fixture tests cover image ordering and omission. The search index includes ingredients for full-text search but never includes instructions or score fields. A shared deterministic `display_title` helper removes terminal generic Chinese `做法` / `的做法` labels from cards, search results, headings and browser titles. It leaves interior uses and descriptive phrases such as `不同做法` intact. This is a rendering rule only: source and normalized titles, publication records, stable identifiers and default ordering are unchanged.
